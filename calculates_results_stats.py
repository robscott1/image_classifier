...
def calculates_results_stats(results_dic):
    ...
    results_stats_dic['pct_correct_dog'] = 0 
    results_stats_dic['pct_correct_notdog'] = 0 
    
    for key in results_dic:
        
        #counts up those keys in dic that had matching image label and classifier label
        if results_dic[key][2] == 1:
            results_stats_dic['n_match'] = results_stats_dic['n_match'] + 1
       
            #classfier label and extracted image label are the same --> correct breed identification
            #this must go inside the above conditional b/c the labels must match
            if results_dic[key][0] == results_dic[key][1]:
                results_stats_dic['n_correct_breed'] += 1
        
        #counts up number of images with dog name and also matched with an element
        #in dognames.txt    
        if results_dic[key][3] == 1:
            results_stats_dic['n_dogs_img'] =  results_stats_dic['n_dogs_img'] + 1
            
            #classifier also claims is-a-dog and matches the pet label --> correctly distinguised dog
            if results_dic[key][4] == 1:
                results_stats_dic['n_correct_dogs'] = results_stats_dic['n_correct_dogs'] + 1
                
        else:
            
            #correctly determines that the image is not a dog
    ...
    #calculates percentages of how the classifier performed
           
    results_stats_dic['n_notdog_img'] = (results_stats_dic['n_images']
